...
export class ApiClient {
  ...
  async request(endpoint, options = {}) {
    ...
    const hasExplicitTeam = options.teamId !== undefined;
    ...
    try {
      ...
      if (response.ok && options.responseType === 'arrayBuffer') return response.arrayBuffer();
      let data = null;
      try {
  ...
  async restorePromptVersion(id, options = {}) {
    return this.request(`/api/prompts/${id}/restore`, {
      method: 'POST',
  ...
  async copyPrompt(promptData, options = {}) {
    return this.request('/api/prompts/copy', {
      method: 'POST',
      body: promptData?.id && (promptData?.skill_package || promptData?.has_skill_package) ? { sourceId: promptData.id } : { promptData },
      teamId: options.teamId,
    });
  }
