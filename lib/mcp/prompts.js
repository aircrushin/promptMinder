...
export function isMcpPromptCreator(prompt, userId) {
  return prompt?.created_by === userId || prompt?.user_id === userId
}

export function getMcpPromptMembership(workspace, prompt) {
  if (!prompt?.team_id) {
    return null
  }

  return workspace.memberships.find((item) => item.team?.id === prompt.team_id) || null
}

export async function loadMcpPromptForAccess(userId, { id, teamId = null, db = defaultDb } = {}) {
  if (!id) {
  ...
  const rows = await db
    .select({
      ...
      lineageId: prompts.lineageId,
      skillPackage: prompts.skillPackage,
...
export async function getMcpPrompt(userId, { id, teamId = null, db = defaultDb } = {}) {
  const { prompt } = await loadMcpPromptForAccess(userId, { id, teamId, db })
