...
export async function ensureLineage(db, { teamId = null, title, userId }) {
  ...
  if (teamId) {
    ...
    lineageConditions.push(isNull(promptLineages.teamId), eq(promptLineages.createdBy, userId))
  }
...
/**
 * Team context: filter by team_id.
 * Personal context: only prompts with null team_id owned by the user.
 * Never allow personal-scope lookups to match team prompts by creator alone.
 */
export function buildPromptAccessScope({ teamId, userId }) {
  if (teamId) {
...
// Personal prompts retain the existing title-based history; teams and Skills use lineage IDs.
export function buildPromptVersionScope({ prompt, teamId, userId }) {
  return and(
    buildPromptAccessScope({ teamId, userId }),
    teamId || prompt.skill_package ? eq(prompts.lineageId, prompt.lineage_id) : eq(prompts.title, prompt.title),
  );
}

export async function getPromptByScope(db, { promptId, teamId, userId, versionOf }) {
  const rows = await db
    .select()
    .from(prompts)
    .where(and(eq(prompts.id, promptId), versionOf
      ? buildPromptVersionScope({ prompt: versionOf, teamId, userId })
      : buildPromptAccessScope({ teamId, userId })))
    .limit(1)
...
export async function createPromptDirect(db, { teamId = null, userId, data, lineageId: existingLineageId = null }) {
  const lineageId = existingLineageId || await ensureLineage(db, {
    teamId,
    title: data.title,
    userId,
  })
  ...
  const result = await db
    ...
    .values({
      ...
      skillPackage: resolveSkillPackage(data),
...
export async function createChangeRequest(db, {
  ...
  const inserted = await db
    ...
    .values({
      ...
      proposedSkillPackage: resolveSkillPackage(proposal),
  ...
  if (isApprove) {
    const insertedPrompt = await db
      ...
      .values({
        ...
        skillPackage: resolveSkillPackage({ content: request.proposed_content, skill_package: request.proposed_skill_package }),
