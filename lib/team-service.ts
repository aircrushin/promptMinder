import { ApiError, assert } from './api-error'
import { eq, and, inArray, count } from 'drizzle-orm'
import { teams, teamMembers } from '@/drizzle/schema/index'
...
export class TeamService {
  ...
  async ensureOwnerMembership(teamId: string, userId: string) {
    const rows = await this.db
      .select()
      .from(teamMembers)
  ...
  async listTeamsForUser(userId: string, { includePending = false }: { includePending?: boolean } = {}) {
  ...
  async requireMembership(teamId: string, userId: string, { allowStatuses = [TEAM_STATUSES.ACTIVE], allowRoles = ACTIVE_ROLES }: { allowStatuses?: string[]; allowRoles?: string[] } = {}) {
  ...
  async assertManager(teamId: string, userId: string) {
    return this.requireMembership(teamId, userId, {
      allowRoles: MANAGER_ROLES
    })
  }

  async assertOwner(teamId: string, userId: string) {
    return this.requireMembership(teamId, userId, {
      allowRoles: [TEAM_ROLES.OWNER]
    })
  }

  async createTeam({ name, description = null, avatarUrl = null, isPersonal = false }: { name: string; description?: string | null; avatarUrl?: string | null; isPersonal?: boolean }, ownerId: string) {
  ...
  async ensureNoPersonalTeam(userId: string) {
    const rows = await this.db
      .select({ id: teams.id })
      .from(teams)
      .where(and(eq(teams.ownerId, userId), eq(teams.isPersonal, true)))
  ...
  async updateTeam(teamId: string, updates: any, actorUserId: string) {
    ...
    const drizzleSet: Record<string, any> = { updatedAt: new Date() }
  ...
  async inviteMember(teamId: string, actorUserId: string, { userId, email, role = TEAM_ROLES.MEMBER }: { userId: string; email: string; role?: string }) {
  ...
  async acceptInvite(teamId: string, userId: string, userEmail: string | null = null) {
  ...
  async updateMember(teamId: string, targetUserId: string, actorUserId: string, { role, status }: { role?: string; status?: string }) {
  ...
  async transferOwnership(teamId: string, actorUserId: string, targetUserId: string) {
